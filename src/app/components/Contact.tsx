...
export default function FormularioPersonaje() {
    ...
    const checkCharacterExists = async (name: string): Promise<boolean> => {
        const res = await fetch('/api/validate-character', {
            method: 'POST',
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full max-w-md bg-grey p-6 rounded shadow">
                ...
                <input
                    {...register('name', {
                        ...
                        validate: async (value) => {
                            ...
                            const res = await checkCharacterExists(normalized);
                            return res || "Character does not exist or is not level 70";
                        },
                    })}
                ...
                {errors.name && (
                    <span className="text-red-600 text-sm">
                        ...
                        {errors.name.type === 'validate' && (errors.name.message as string)}
                    </span>
                )}
